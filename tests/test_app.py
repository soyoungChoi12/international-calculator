...
def test_breakfast_checkbox_is_visible():
    ...
    boost = next(widget for widget in at.radio if widget.label == "숙박상한")
    assert "1.5배" in list(boost.options)
...
def test_changing_approval_date_loads_that_days_fx(monkeypatch):
    ...
    def fake_fetch(when: date):
        rates = {
            date(2026, 8, 28): 1404.65,
            date(2026, 9, 1): 1399.57,
        }
        rate = rates.get(when, 1382.88)
    ...
    assert any("1,399.57원" in caption.value and "2026-09-01" in caption.value for caption in at.caption)
