...
def test_lodging_boost_note_in_hwp_budget():
    result = calculate_travel(
        TravelInput(
            role="팀장 및 팀원",
            grade="나",
            ...
            lodging_actual_krw=0,
            preparation_krw=150_000,
            stays=[StayInput("일본", "교토", 3, "나", stay_days=4, lodging_boosted=True)],
        )
    )
    fields = build_hwp_fields(
        result,
    ...
    assert "숙박비 : 847,320원($552, 숙박 3박 1.5배)" in fields["budget"]
